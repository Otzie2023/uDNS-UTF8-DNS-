...
pub fn load_zones_config(path: &str, start_time: DateTime<Utc>) -> Result<(ZonesFile, Vec<ZoneJson>), Box<dyn Error>> {
                    ...
                    Ok(dt_fixed) => {
                        ...
                        if !soa.expire.is_infinite() {
                            ...
                            if age_seconds > expire_seconds {
                                println!(
                                    "Warnung: Zone '{}' wäre verfallen (age {}s > expire {}s), wird jedoch geladen da 'enabled': true.",
                                    entry.domain, age_seconds, expire_seconds
                                );
                                // Do not skip loading here; an explicit `enabled: true` entry must be loaded.
                            } else {
                                println!(
